...
function sanitizeCommand(cmd) {
  // Only allow alphanumeric, dash, underscore, spaces, and common git subcommands
  return cmd.replace(/[^a-zA-Z0-9\-_ .]/g, '').trim();
}
...
function runCommand(cmd) {
  try {
    ...
    const allowed = ['git status', 'git log', 'ls', 'cat', 'head', 'tail', 'date', 'whoami'];
    const parts = sanitizedCmd.split(' ');
    const action = parts[0];
    
    // Check if command matches allowed patterns
    const isAllowed = allowed.some(allowedCmd => 
      sanitizedCmd.startsWith(allowedCmd) || sanitizedCmd === allowedCmd
    );
    ...
    const output = execSync(sanitizedCmd, { 
      cwd: workspace, 
      encoding: 'utf8',
      timeout: 5000
    });
...
const server = http.createServer((req, res) => {
  ...
  // Check API key for non-GET requests
  ...
  if (req.method === 'GET') {
    ...
    // /api/command - run a command via query param (still needs key for sensitive operations)
